...
def CreateSinogram(image, flag=1, plot=False, **kwargs):
...
def Raw2Sinogram(
    ...
    **kwargs
    ...
    rawImageArray = np.fromfile(
        rawInFile, dtype=np.uint16, count=imageHeight * imageWidth
    )
    ...
    if plot:
        ...
        ax1.imshow(rawRGBarray, cmap=plt.cm.Greys_r)
...
def GetImages(
    fileNameList, mode=0, imageHeight=400, imageWidth=400, imageChannels=1, **kwargs
):

    if mode == 0:
        SinogramList = np.zeros(
            (len(fileNameList), imageHeight, imageWidth), dtype=np.uint16
        )
    if mode == 1:
        SinogramList = np.zeros(
            (len(fileNameList), imageHeight, imageWidth, imageChannels), dtype=np.uint16
        )
    elif mode != 0 and mode != 1:
        return -1

    ReconstructedSinogramArray = np.zeros(
        (len(fileNameList), imageHeight, imageWidth), dtype=np.uint16
    )

    SinogramThetaList = [
        Raw2Sinogram(x, imageWidth, imageHeight, mode, **kwargs) for x in fileNameList
    ]
    SinogramArray = np.array([x[0][:, :] for x in SinogramThetaList])
    ReconstructedSinogramArray = np.array(
        [ReconstructSinogram(x) for x in SinogramThetaList]
    )
...
if __name__ == "__main__":
    Raw2Sinogram(
        rawFilename="../data/1/IE1705794_P406.i180073.raw",
        imageHeight=400,
        imageWidth=400,
        mode=1,
    )
